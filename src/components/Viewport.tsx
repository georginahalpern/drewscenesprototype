...
function ensureSceneLoadersRegistered(): void {
  ...
  // Enables Mesh.createInstance / registerInstancedBuffer, which are
  // side-effect stubs in the tree-shakable ("pure") build and otherwise throw.
...
export default function Viewport({
  ...
  // Base (unselected) per-instance color for every primitive prim, keyed by
  // prim id. The selection effect tints the live instancedBuffers.color toward
  // SELECTION_COLOR and restores from here on deselect.
  ...
  // Currently-tinted primitive instances (selection highlight rides the
  // per-instance color buffer, since InstancedMesh has no renderOverlay).
  ...
  useEffect(() => {
    ...
    // Pass 3: dispose meshes whose prims are gone.
